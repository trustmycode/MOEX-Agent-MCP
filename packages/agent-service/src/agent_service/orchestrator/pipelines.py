...
def get_pipeline(scenario_type: ScenarioType) -> ScenarioPipeline:
    ...
    # Шаги деградированного режима изменяются при выполнении запроса. Возвращаем
    # независимую копию, чтобы параллельные запросы не меняли общий реестр.
...
def list_pipelines() -> list[ScenarioPipeline]:
    ...
    """
    return [deepcopy(pipeline) for pipeline in _PIPELINE_REGISTRY.values()]
