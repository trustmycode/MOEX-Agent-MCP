...
- [ ] Существует интерфейс `FundamentalsDataProvider` с чётко определёнными методами.
- [ ] Реализован класс `MoexIssFundamentalsProvider`, возвращающий стабильные доменные модели для выбранных тикеров.
- [ ] Все сценарии, использующие фундаментал (5/7/9), работают через этот интерфейс, а не напрямую через MOEX ISS.
- [ ] Написано минимум 2 unit-теста, использующих сохранённые ответы MOEX ISS.
- [ ] В документации MCP описано, какие именно метрики гарантированно доступны из `FundamentalsDataProvider` в MVP.
