...
### 4.1. Архитектурные требования (Multi-Agent)

**FR-A-ARCH-1. Паттерн Orchestrator + Subagents**

Агент **обязан** быть реализован по паттерну **Orchestrator-Workers (Multi-Agent)**:

- **OrchestratorAgent** — центральный координатор, принимает A2A-запрос, определяет сценарий и делегирует задачи сабагентам.
- **Subagents** — специализированные агенты с изолированными промптами и чёткой ответственностью.

Обоснование: паттерн обеспечивает изоляцию контекста, лучшую тестируемость и возможность независимого масштабирования.

**FR-A-ARCH-2. Обязательные сабагенты**

Система должна включать следующие сабагенты:

| Сабагент | Ответственность | Зависимости |
|----------|-----------------|-------------|
| `ResearchPlannerSubagent` | Определение scenario_type, построение плана | LLM |
| `MarketDataSubagent` | Работа с moex-iss-mcp | MCP (moex-iss) |
| `RiskAnalyticsSubagent` | Работа с risk-analytics-mcp | MCP (risk-analytics) |
| `DashboardSubagent` | Формирование RiskDashboardSpec | — |
| `ExplainerSubagent` | Генерация текстового отчёта | LLM |
| `KnowledgeSubagent` | Работа с kb-rag-mcp | MCP (kb-rag) |

**FR-A-ARCH-3. Отказ от монолитного планировщика**

Система **не должна** использовать монолитный планировщик (`Planner` + `ToolOrchestrator` как единый компонент). Вместо этого:

- Планирование делегируется `ResearchPlannerSubagent`.
- Исполнение делегируется специализированным сабагентам.
- Координация осуществляется `OrchestratorAgent`.

**FR-A-ARCH-4. AgentContext и разделяемое состояние**

Все сабагенты должны получать и обновлять разделяемый `AgentContext`, который содержит:

- `user_query`, `locale`, `user_role` — входные данные запроса.
- `scenario_type` — определённый тип сценария.
- `intermediate_results` — промежуточные результаты от сабагентов.
- `errors` — список ошибок для graceful degradation.

**FR-A-ARCH-5. AgentRegistry**

Должен существовать `AgentRegistry` — реестр сабагентов, который:

- Регистрирует и инициализирует сабагентов.
- Предоставляет доступ к сабагентам по имени/роли.
- Управляет зависимостями сабагентов (MCP-клиенты, LLM-клиент).
...
### 6.5. Мультиагентная архитектура (NFR)

**NFR-MA-1. Изоляция промптов**

- Каждый сабагент **должен** иметь собственный системный промпт, изолированный от других сабагентов.
- Промпт сабагента **не должен** содержать инструкции для задач других сабагентов.
- Это предотвращает «confusion» LLM и улучшает качество ответов.

**NFR-MA-2. Управление контекстом**

- Размер контекста каждого сабагента **должен** быть ограничен только релевантными данными:
  - `ResearchPlannerSubagent` получает только `user_query`.
  - `MarketDataSubagent` получает только параметры запроса к MCP.
  - `ExplainerSubagent` получает метрики и (опционально) RAG-snippets.
- Промежуточные результаты передаются через `AgentContext.intermediate_results`, а не через LLM-контекст.

**NFR-MA-3. Трейсинг цепочки сабагентов**

- Каждый сабагент **должен** создавать child-span от parent-span Orchestrator.
- Span должен содержать атрибуты: `subagent.name`, `scenario_type`, `success`.
- Это обеспечивает прозрачность и отладку цепочки вызовов.

**NFR-MA-4. Graceful Degradation**

- При ошибке некритичного сабагента (например, `KnowledgeSubagent`) система **должна** продолжить выполнение с частичными результатами.
- Пользователь должен получить ответ с пометкой об ограничениях (например, «методические пояснения недоступны»).
- Критичные сабагенты (определённые в плане как `required=true`) при ошибке прерывают выполнение с понятным сообщением.

**NFR-MA-5. Тестируемость сабагентов**

- Каждый сабагент **должен** быть тестируемым изолированно:
  - Unit-тесты с mock-зависимостями (MCP, LLM).
  - Контрактные тесты для проверки входов/выходов.
- Интеграционные тесты проверяют взаимодействие Orchestrator ↔ Subagents.
